...
class GPT(nn.Module):
    ...
    def init_weights(self):
        ...
        # Cast embeddings to bf16 (Ampere+) or fp32 (pre-Ampere, required by GradScaler)
        embed_dtype = (
            torch.bfloat16
...
FINAL_LR_FRAC = 0.035  # final LR as fraction of initial
...
def get_muon_momentum(step):
    frac = min(step / 300, 1)
    return (1 - frac) * 0.85 + frac * 0.85


def get_weight_decay(progress):
    return WEIGHT_DECAY if progress >= 0.78 else 0.0
